...
export async function expectResult(
  ...
  expect(duckdb.result_statement_type(result)).toBe(
    expectedResult.statementType ?? duckdb.StatementType.SELECT
  );
  expect(duckdb.result_return_type(result)).toBe(
    expectedResult.resultType ?? duckdb.ResultType.QUERY_RESULT
  );
  ...
  for (let col = 0; col < expectedResult.columns.length; col++) {
    ...
    expect(duckdb.column_type(result, col), `${col}`).toBe(
      expectedColumn.logicalType.typeId
    );
    expectLogicalType(
      duckdb.column_logical_type(result, col),
      expectedColumn.logicalType,
      `col ${col}`
    );
  }
  if (expectedResult.chunkCount != undefined && expectedResult.chunkCount > 0) {
    for (
      let chunkIndex = 0;
      chunkIndex < expectedResult.chunkCount;
      chunkIndex++
    ) {
      const chunk = duckdb.result_get_chunk(result, chunkIndex);
      expectChunk(
        chunk,
        expectedResult.chunks[chunkIndex],
